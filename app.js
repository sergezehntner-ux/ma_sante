...
openTreatmentForm.onclick=()=>{if(!db.pharmacy.length)return alert('Ajoute ou importe d’abord les médicaments dans Pharmacie.');resetTreatment();treatmentFormPanel.classList.add('open')};
document.getElementById('addOneOffTreatment').onclick=()=>{
 ...
 treatmentFormPanel.classList.add('open');
...
openContactForm.onclick=()=>{resetContactForm();contactFormPanel.classList.add('open')};
...
saveContact.onclick=()=>{
 ...
 contactFormPanel.classList.remove('open');save();fillPrescriberSelect();renderContacts();revealSavedRow(c.id,'contact');
...
function duplicateContact(id){
 ...
 contactFormPanel.classList.add('open');
}
...
function refreshPharmacyCompendiumNames(){
 const dl=document.getElementById('phCompendiumNames');if(!dl)return;
 const names=[...new Set(compendiumMedicines().map(p=>p.name).filter(Boolean))].sort(alpha);
 dl.innerHTML=names.map(n=>`<option value="${esc(n)}"></option>`).join('');
}
function refreshContactNameSuggestions(){
 const dl=document.getElementById('contactNameSuggestions');if(!dl)return;
 const vals=[...new Set(db.contacts.flatMap(c=>[c.lastName,c.reference]).filter(Boolean))].sort(alpha);
...
openPharmacyForm.onclick=()=>{resetPharmacy();pharmacyFormPanel.classList.add('open')};cancelPharmacy.onclick=()=>pharmacyFormPanel.classList.remove('open');
...
function editPharmacy(id){const p=normalizeLots(pharmacyItem(id));if(!p)return;resetPharmacy();pharmacyEditId.value=p.id;pharmacyFormTitle.textContent='Modifier le produit';fillPhItemType(p.itemType||'product',p.serviceType||'');phStockFields.classList.remove('hidden');phName.value=p.name;phStrength.value=p.strength||'';dynamicSelect('phUnit','phUnitOther','unit',p.unit||'');phThreshold.value=p.threshold||0;phLots.innerHTML='';(p.lots.length?p.lots:[{qty:0,expiry:''}]).forEach(l=>addLotRow(l.qty,l.expiry));phInformation.value=p.information||'';phPhotoStatus.textContent=(p.imageKey||p.photo)?'Photo enregistrée — tu peux la voir, la remplacer ou la supprimer.':'';refreshPharmacyImageButtons(!!(p.imageKey||p.photo));pharmacyFormPanel.classList.add('open')}
