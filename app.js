...
savePharmacy.onclick=async()=>{try{if(!phName.value.trim())return alert('Indique le nom.');const old=pharmacyItem(pharmacyEditId.value);let photo=old?.photo||'';if(phPhoto.files[0]){if(phPhoto.files[0].size>1500000)return alert('Photo trop volumineuse. Choisis une image de moins de 1,5 Mo.');photo=await fileToDataURL(phPhoto.files[0])}const lots=[...phLots.children].map(r=>({id:uid(),qty:Number(r.querySelector('.lotQty').value||0),expiry:r.querySelector('.lotExpiry').value||''})).filter(l=>l.qty>0);const p=normalizeLots({id:pharmacyEditId.value||uid(),name:phName.value.trim(),strength:phStrength.value.trim(),unit:selectedOrOther('phUnit','phUnitOther')||'unité',threshold:Number(phThreshold.value||0),lots,information:phInformation.value.trim(),photo});const ix=db.pharmacy.findIndex(x=>x.id===p.id);if(ix>=0)db.pharmacy[ix]=p;else db.pharmacy.push(p);if(save()){pharmacyFormPanel.classList.remove('open');
 if(prescriptionDraft?.mode==='product'){
  const d=prescriptionDraft;prescriptionDraft=null;
  document.querySelectorAll('nav button').forEach(x=>x.classList.toggle('active',x.dataset.view==='prescriptions'));
  document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active',v.id==='prescriptions'));
  resetPrescription();prescriptionFormPanel.classList.add('open');fillProductSelect('prescriptionProduct',p.id);fillPrescriberSelect(d.prescriberContactId||'');
  issueDate.value=d.issueDate||isoDay();validUntil.value=d.validUntil||'';renewalsAllowed.value=d.renewalsAllowed||0;renewalsUsed.value=d.renewalsUsed||0;prescriptionNotes.value=d.notes||'';
  prescriptionFormPanel.scrollIntoView({behavior:'smooth'});
 }
}}catch(e){alert("Impossible d’enregistrer l’image : "+(e.message||e))}}
