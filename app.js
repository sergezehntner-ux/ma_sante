const KEY='ma-sante-v0192';
const OLD_KEYS=['ma-sante-v0191','ma-sante-v019','ma-sante-v017','ma-sante-v0183','ma-sante-v0182','ma-sante-v0171','ma-sante-v016','ma-sante-v015','ma-sante-v014','ma-sante-v013','ma-sante-v012','ma-sante-v011','ma-sante-v01'];
function uid(){return(globalThis.crypto&&crypto.randomUUID)?crypto.randomUUID():'id-'+Date.now()+'-'+Math.random().toString(16).slice(2)}
function freshDefault(){return{schemaVersion:3,treatments:[],pharmacy:[],takes:{},history:[],measures:[],measureHistory:[],prescriptions:[]}}
function migrate(data){if(!data||typeof data!=='object')data=freshDefault();data.schemaVersion=3;data.pharmacy=Array.isArray(data.pharmacy)?data.pharmacy:[];data.treatments=Array.isArray(data.treatments)?data.treatments:[];data.takes=data.takes||{};data.history=Array.isArray(data.history)?data.history:[];data.measures=Array.isArray(data.measures)?data.measures:[];data.measureHistory=Array.isArray(data.measureHistory)?data.measureHistory:[];data.prescriptions=Array.isArray(data.prescriptions)?data.prescriptions:[];
...
function fileToDataURL(file){return new Promise((res,rej)=>{if(!file)return res('');const r=new FileReader();r.onerror=rej;r.onload=()=>res(r.result);r.readAsDataURL(file)})}
savePharmacy.onclick=async()=>{try{if(!phName.value.trim())return alert('Indique le nom.');const old=pharmacyItem(pharmacyEditId.value);let photo=old?.photo||'';if(phPhoto.files[0]){if(phPhoto.files[0].size>1500000)return alert('Photo trop volumineuse. Choisis une image de moins de 1,5 Mo.');photo=await fileToDataURL(phPhoto.files[0])}const lots=[...phLots.children].map(r=>({id:uid(),qty:Number(r.querySelector('.lotQty').value||0),expiry:r.querySelector('.lotExpiry').value||''})).filter(l=>l.qty>0);const p=normalizeLots({id:pharmacyEditId.value||uid(),name:phName.value.trim(),strength:phStrength.value.trim(),unit:selectedOrOther('phUnit','phUnitOther')||'unité',threshold:Number(phThreshold.value||0),lots,information:phInformation.value.trim(),photo});const ix=db.pharmacy.findIndex(x=>x.id===p.id);if(ix>=0)db.pharmacy[ix]=p;else db.pharmacy.push(p);if(save())pharmacyFormPanel.classList.remove('open')}catch(e){alert("Impossible d’enregistrer l’image : "+(e.message||e))}}
function editPharmacy(id){const p=normalizeLots(pharmacyItem(id));if(!p)return;resetPharmacy();pharmacyEditId.value=p.id;pharmacyFormTitle.textContent='Modifier le produit';phName.value=p.name;phStrength.value=p.strength||'';dynamicSelect('phUnit','phUnitOther','unit',p.unit||'');phThreshold.value=p.threshold||0;phLots.innerHTML='';(p.lots.length?p.lots:[{qty:0,expiry:''}]).forEach(l=>addLotRow(l.qty,l.expiry));phInformation.value=p.information||'';phPhotoStatus.textContent=p.photo?'Photo enregistrée — sélectionner une nouvelle image pour la remplacer.':'';pharmacyFormPanel.classList.add('open');pharmacyFormPanel.scrollIntoView({behavior:'smooth'})}
function viewPharmacy(id){const p=normalizeLots(pharmacyItem(id));if(!p)return;pharmacyDetailTitle.textContent=p.name+(p.strength?' · '+p.strength:'');pharmacyDetailBody.innerHTML=`${p.photo?`<img class="photo-preview" src="${p.photo}">`:''}<div><strong>Unité :</strong> ${esc(p.unit)}<br><strong>Stock :</strong> ${p.stock}<br><strong>Seuil :</strong> ${p.threshold}<br><strong>Prochaine péremption :</strong> ${esc(p.expiry||'—')}<br><strong>Informations :</strong> ${esc(p.information||'—')}</div><h4>Boîtes / lots</h4>${p.lots.length?p.lots.map(l=>`<div class="detail-lot">${l.qty} ${esc(p.unit)} · péremption ${esc(l.expiry||'non indiquée')}</div>`).join(''):'<div class="muted">Aucun stock.</div>'}`;openModal('pharmacyDetailModal')}
function showPharmacyPhoto(id){const p=pharmacyItem(id);if(!p?.photo)return alert('Aucune photo.');pharmacyDetailTitle.textContent=p.name;pharmacyDetailBody.innerHTML=`<img class="photo-preview" src="${p.photo}">`;openModal('pharmacyDetailModal')}
...
function renderPrescriptions(){fillProductSelect('prescriptionProduct');const today=isoDay(),list=[...db.prescriptions].sort((a,b)=>alpha(pharmacyItem(a.pharmacyId)?.name,pharmacyItem(b.pharmacyId)?.name));prescriptionList.innerHTML=list.length?list.map(r=>{const p=pharmacyItem(r.pharmacyId),expired=r.validUntil&&r.validUntil<today;return`<div class="card compact-card prescription-row"><div><strong>${esc(p?.name||'Produit inconnu')}</strong><div class="muted">${esc(r.prescriber||'')} · ${esc(r.issueDate||'')} → ${esc(r.validUntil||'sans limite')} · renouvellements ${r.renewalsUsed||0}/${r.renewalsAllowed||0}</div><div class="${expired?'status-expired':'status-valid'}">${expired?'Expirée':'Valide / à vérifier'}</div></div><div class="actions">${r.hasPdf?`<button class="secondary icon-btn" onclick="openPrescriptionPdf('${r.id}')">Voir PDF</button>`:''}<button class="secondary icon-btn" onclick="editPrescription('${r.id}')">Modifier</button><button class="danger icon-btn" onclick="deletePrescription('${r.id}')">×</button></div></div>`}).join(''):'<div class="card compact-card">Aucune ordonnance.</div>'}
function resetPrescription(){prescriptionEditId.value='';prescriptionFormTitle.textContent='Ajouter une ordonnance';fillProductSelect('prescriptionProduct');prescriber.value='';issueDate.value=isoDay();validUntil.value='';renewalsAllowed.value=0;renewalsUsed.value=0;prescriptionPdf.value='';prescriptionPdfStatus.textContent='Aucun PDF associé.';viewPrescriptionPdf.classList.add('hidden');removePrescriptionPdf.classList.add('hidden');pdfRemovePending=false;prescriptionNotes.value=''}openPrescriptionForm.onclick=()=>{resetPrescription();prescriptionFormPanel.classList.add('open');prescriptionFormPanel.scrollIntoView({behavior:'smooth'})};cancelPrescription.onclick=()=>prescriptionFormPanel.classList.remove('open');
prescriptionProduct.onchange=()=>{
 if(prescriptionProduct.value!=='__NEW__')return;
 prescriptionDraft={id:prescriptionEditId.value,prescriber:prescriber.value,issueDate:issueDate.value,validUntil:validUntil.value,renewalsAllowed:renewalsAllowed.value,renewalsUsed:renewalsUsed.value,notes:prescriptionNotes.value};
...
savePrescription.onclick=async()=>{
 ...
 try{
  if(pdfRemovePending)await pdfDel(id);
  if(file)await pdfPut(id,file);
  const r={id,pharmacyId:prescriptionProduct.value,prescriber:prescriber.value.trim(),issueDate:issueDate.value,validUntil:validUntil.value,renewalsAllowed:Number(renewalsAllowed.value||0),renewalsUsed:Number(renewalsUsed.value||0),notes:prescriptionNotes.value.trim(),hasPdf:pdfRemovePending?false:(file?true:!!old?.hasPdf)};
  const ix=db.prescriptions.findIndex(x=>x.id===id);if(ix>=0)db.prescriptions[ix]=r;else db.prescriptions.push(r);prescriptionFormPanel.classList.remove('open');save();
 }catch(e){alert('Impossible d’enregistrer le PDF : '+(e.message||e))}
};function editPrescription(id){const r=db.prescriptions.find(x=>x.id===id);if(!r)return;resetPrescription();prescriptionEditId.value=r.id;prescriptionFormTitle.textContent='Modifier l’ordonnance';fillProductSelect('prescriptionProduct',r.pharmacyId);prescriber.value=r.prescriber||'';issueDate.value=r.issueDate||'';validUntil.value=r.validUntil||'';renewalsAllowed.value=r.renewalsAllowed||0;renewalsUsed.value=r.renewalsUsed||0;prescriptionNotes.value=r.notes||'';prescriptionFormPanel.classList.add('open');prescriptionFormPanel.scrollIntoView({behavior:'smooth'});if(r.hasPdf){prescriptionPdfStatus.textContent='PDF enregistré';viewPrescriptionPdf.classList.remove('hidden');removePrescriptionPdf.classList.remove('hidden')}}function deletePrescription(id){if(confirm('Supprimer cette ordonnance ?')){db.prescriptions=db.prescriptions.filter(x=>x.id!==id);save()}}
